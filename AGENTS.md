...
## Harness
- Aplique o harness global em `/Users/igorcampana/.codex/AGENTS.md`.
- Use GSD Core como workflow primário para trabalho substancial.
- Use este `AGENTS.md` como contrato operacional local.
- Trate planos, specs e docs de referência como legado. Não migre nem apague sem pedido explícito.
...
## Comandos Úteis
- O projeto não utiliza bundlers (Vite/Webpack). É HTML/JS puro. Não há `npm install`.
- Desenvolvimento local: sirva os arquivos estáticos com "Live Server" (VS Code) ou um servidor HTTP simples.
- Deploy: `git push origin main` (gera deploy automático e contínuo na Vercel).
